...
//Stack methods
template <typename T>
void CS2420Stack<T>::push(const T& item) {
	if (currentSize >= capacity) {
		cout << "The stack is full, unable to push item." << endl;
		return;
	}
...
template <typename T>
void CS2420Stack<T>::push(T&& item) {
	if (currentSize >= capacity) {
		cout << "The stack is full, unable to push item." << endl;
		return;
	}
	arr[currentSize] = std::move(item);
	currentSize++;
}
...
T CS2420Stack<T>::popAndCopy() {
	if (currentSize == 0) {
		...
		currentSize--; //Keep this decrement above following line, otherwise the whole thing goes haywire.
...
T&& CS2420Stack<T>::popAndMove() {
	if (currentSize == 0) {
		...
		currentSize--; //Decrement needs to come first
		int sizeCopy = currentSize;
		return std::move(arr[sizeCopy]);

	}
}

template <typename T>
unsigned int CS2420Stack<T>::size() const {
	return currentSize;
}
...
T CS2420Queue<T>::popFront() {
	...
	unsigned int frontCopy = front;
	currentSize--;
	front++;
	front = front % capacity; //Thanks Prof. Peterson for this, a lot cleaner than an if statement.
	return arr[frontCopy];
}

template <typename T>
void CS2420Queue<T>::pushBack(const T& item) {
	if (currentSize >= capacity) {
		cout << "The queue is full, unable to push item." << endl;
		return;
	}
	arr[back] = item;
	back++;
...
template <typename T>
unsigned int CS2420Queue<T>::size() const {
	return currentSize;
}
